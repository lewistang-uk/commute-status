...
The TfL app is useful, but sometimes lacks information. By using TfL's API as a source of train data, potential delays can be identified before official confirmation, as well as being able to check if the next train is direct.
...
A hypothesis was formed: trains on the Edgware Road branch do not operate on the main branch. To investigate, train numbers were collected at High Street Kensington (Edgware Road branch), as well as at Gloucester Road. The sets of train numbers at both stations had empty intersection over a three-hour period.

In the Streamlit app, the set of train numbers observed at High Street Kensington is queried. If the number of the next train at Southfields is not in this set, then the train is marked as "likely direct".
